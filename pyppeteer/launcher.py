...
class BrowserRunner:
    ...
    def start(self, **kwargs: LaunchOptions) -> None:
        ...
        logger.debug(
            f'Calling process: {self.executable_path} {" ".join(self.process_args)}'
        )
        self.proc = subprocess.Popen(
            [str(self.executable_path), *self.process_args], **process_opts
        )
        self._closed = False
        ...
        if self.proc and not self._closed and self.proc.returncode is not None:
            ...
            try:
                ...
                if sys.platform.startswith("win"):
                    subprocess.Popen(
                        ["taskkill", "/PID", str(self.proc.pid), "/F"], shell=True
                    ).communicate()
    ...
    async def setupConnection(
        ...
        if usePipe:
            raise NotImplementedError(
                "Communication via pipe not supported at this time"
            )
        ...
        self.connection = Connection(
            url=browser_ws_endpoint, transport=transport, delay=slowMo
        )
...
class BaseBrowserLauncher:
    ...
    def __init__(
        self, projectRoot: Union[Path, str] = None, preferredRevision: str = None
    ):
    ...
    async def connect(
        ...
        if transport:
            ...
            connection = Connection(
                browserWSEndpoint, transport=transport, delay=slowMo
            )
...
class ChromeLauncher(BaseBrowserLauncher):
    ...
    async def launch(
        self, **kwargs: Union[LaunchOptions, ChromeArgOptions, BrowserOptions]
    ) -> Browser:
        ...
        if not any(x.startswith(f"--user-data-dir") for x in chrome_args):
            profile_path = tempfile.TemporaryDirectory(
                prefix="pyppeteer2_chrome_profile_"
            )
        ...
        try:
            ...
            logger.error(
                f"error occurred while trying to launch browser: {original_exception}"
            )
...
class FirefoxLauncher(BaseBrowserLauncher):
    ...
    async def launch(
        ...
        if not executablePath:
            missing_text, executablePath = resolveExecutablePath(
                self.projectRoot, self.preferredRevision
            )
            if missing_text:
                raise RuntimeError(missing_text)
    ...
    def _create_profile(
        ...
        serialized_prefs = [
            f"user_pref({json.dumps(key)}, {json.dumps(val)}"
...
def waitForWSEndpoint(
    proc: subprocess.Popen, timeout: Optional[float], preferredRevision: Optional[str]
) -> str:
...
def resolveExecutablePath(
    projectRoot: Path, preferred_revision: str
