...
class TestEvaluate(BaseTestCase):
    ...
    async def test_after_framenavigation(self):
        ...
        assert frameEvaluation.result() == 42
    ...
    async def test_fail_window_object(self):
        assert await self.page.evaluate('() => window') is None
        assert await self.page.evaluate('() => [Symbol("foo4")]') is None
    ...
    async def test_element_handle_disposed(self):
        ...
        assert 'JSHandle is disposed' in cm.exception.args[0]
    ...
    async def test_nice_error_after_navigation(self):
        ...
        assert 'navigation' in cm.exception.args[0]
...
class TestOfflineMode(BaseTestCase):
    ...
    async def test_emulate_navigator_offline(self):
        assert await self.page.evaluate('window.navigator.onLine')
        await self.page.setOfflineMode(True)
        assert not await self.page.evaluate('window.navigator.onLine')
        await self.page.setOfflineMode(False)
        assert await self.page.evaluate('window.navigator.onLine')
...
class TestWaitFor(BaseTestCase):
    ...
    async def test_wait_for_error_type(self):
        ...
        assert 'Unsupported target type' in cm.exception.args[0]
...
class TestConsole(BaseTestCase):
    ...
    async def test_console_event(self):
        ...
        msg = messages[0]
        assert msg.type == 'log'
        assert msg.text == 'hello 5 JSHandle@object'
        assert await msg.args[0].jsonValue() == 'hello'
        assert await msg.args[1].jsonValue() == 5
        assert await msg.args[2].jsonValue() == {'foo': 'bar'}
...
console.log(Promise.resolve('should not wait until resolved!'));
    ...
    async def test_console_window(self):
        ...
        assert len(messages) == 1
        msg = messages[0]
        assert msg.text == 'JSHandle@object'
    ...
    async def test_trigger_correct_log(self):
        ...
        assert len(messages) == 1
        message = messages[0]
        assert 'No \'Access-Control-Allow-Origin\'' in message.text
...
class TestMetrics(BaseTestCase):
    def checkMetrics(self, metrics):
        ...
        for name, value in metrics.items():
            assert name in metrics_to_check
            assert value >= 0
...
class TestGoto(BaseTestCase):
    ...
    async def test_get_http(self):
        ...
        assert self.page.url == 'http://example.com/'
    ...
    async def test_goto_fail_204(self):
        ...
        assert 'net::ERR_ABORTED' in cm.exception.args[0]
    ...
    async def test_redirect(self):
        ...
        assert response.ok
        assert response.url == self.url + 'redirect2'
    ...
    async def test_show_url_in_error_message(self):
        ...
        assert url in cm.exception.args[0]
...
class TestGoBack(BaseTestCase):
    ...
    async def test_back(self):
        ...
        assert response.ok
        assert 'empty' in response.url

        response = await self.page.goForward()
        assert response.ok
        assert 'static/textarea.html' in response.url
    ...
    async def test_history_api(self):
        ...
        assert self.page.url == self.url + 'first.html'
        await self.page.goBack()
        assert self.page.url == self.url + 'empty'
...
class TestRequest(BaseTestCase):
    ...
    async def test_request(self):
        ...
        assert requests[0].url == self.url + 'empty'
        assert requests[0].frame == self.page.mainFrame
        assert requests[0].frame.url == self.url + 'empty'
        assert requests[1].url == self.url + 'empty'
        assert requests[1].frame == self.page.frames[1]
        assert requests[1].frame.url == self.url + 'empty'
...
class TestExtraHTTPHeader(BaseTestCase):
    ...
    async def test_extra_http_header(self):
        ...
        assert len(requests) == 1
        assert requests[0].headers['foo'] == 'bar'

    @sync
    async def test_non_string_value(self):
        with pytest.raises(TypeError) as e:
            await self.page.setExtraHTTPHeaders({'foo': 1})
        assert 'Expected value of header "foo" to be string' in e.exception.args[0]
...
class TestSetBypassCSP(BaseTestCase):
    ...
    async def test_bypass_csp_meta_tag(self):
        ...
        assert await self.page.evaluate('window.__injected') is None
...
class TestUrl(BaseTestCase):
    ...
    async def test_url(self):
        await self.page.goto('about:blank')
        assert self.page.url == 'about:blank'
        await self.page.goto(self.url + 'empty')
        assert self.page.url == self.url + 'empty'
...
class TestViewport(BaseTestCase):
    ...
    async def test_viewport(self):
        assert self.page.viewport == {'width': 800, 'height': 600}
        await self.page.setViewport({'width': 123, 'height': 456})
        assert self.page.viewport == {'width': 123, 'height': 456}
    ...
    async def test_mobile_emulation(self):
        ...
        assert await self.page.evaluate('window.innerWidth') == 375
        await self.page.setViewport({'width': 400, 'height': 300})
        assert await self.page.evaluate('window.innerWidth') == 400

    @sync
    async def test_touch_emulation(self):
        await self.page.goto(self.url + 'static/mobile.html')
        assert not await self.page.evaluate('"ontouchstart" in window')
        await self.page.setViewport(self.iPhoneViewport)
        assert await self.page.evaluate('"ontouchstart" in window')
        ...
        assert await self.page.evaluate(dispatchTouch) == 'Received touch'
    ...
    async def test_detect_touch_viewport_touch(self):
        ...
        assert await self.page.evaluate('() => Modernizr.touchevents')  # noqa: E501
...
class TestEmulateMedia(BaseTestCase):
    ...
    async def test_emulate_media(self):
        assert await self.page.evaluate('matchMedia("screen").matches')
        assert not await self.page.evaluate('matchMedia("print").matches')
        await self.page.emulateMedia('print')
        assert not await self.page.evaluate('matchMedia("screen").matches')
        assert await self.page.evaluate('matchMedia("print").matches')
    ...
    async def test_emulate_media_bad_arg(self):
        ...
        assert cm.exception.args[0] == 'Unsupported media type: bad'
...
class TestEvaluateOnNewDocument(BaseTestCase):
    ...
    async def test_csp(self):
        await self.page.evaluateOnNewDocument('() => window.injected = 123')
        await self.page.goto(self.url + 'csp')
        assert await self.page.evaluate('window.injected') == 123
        with pytest.raises(ElementHandleError):
            await self.page.addScriptTag(content='window.e = 10;')
        assert await self.page.evaluate('window.e') is None
...
class TestCacheEnabled(BaseTestCase):
    ...
    async def test_cache_enable_disable(self):
        ...
        await self.page.reload(waitUntil='networkidle2')
        assert not responses.get('one-style.css').fromCache
...
class TestSelect(BaseTestCase):
    ...
    async def test_select_first_item(self):
        ...
        assert await self.page.evaluate('result.onInput') == ['blue']
        assert await self.page.evaluate('result.onChange') == ['blue']
    ...
    async def test_select_multiple(self):
        ...
        _input = await self.page.evaluate('result.onInput')
        assert _input == ['blue', 'green', 'red']
        change = await self.page.evaluate('result.onChange')
        assert change == ['blue', 'green', 'red']
    ...
    async def test_return_selected_elements(self):
        ...
        assert len(result) == 3
        assert set(result) == {'blue', 'black', 'magenta'}
...
class TestBrowser(BaseTestCase):
    ...
    async def test_get_browser(self):
        assert self.page.browser is self.browser
