...
class TestClick(BaseTestCase):
    ...
    async def test_click_wrapped_links(self):
        ...
        self.assertEqual(self.page.url, self.url + 'static/wrappedlink.html#clicked')
    ...
    async def test_scroll_and_click(self):
        ...
        self.assertEqual(await self.page.evaluate('document.querySelector("#button-5").textContent'), 'clicked')
        await self.page.click('#button-80')
        self.assertEqual(await self.page.evaluate('document.querySelector("#button-80").textContent'), 'clicked')
    ...
    async def test_select_text_by_mouse(self):
        ...
        await self.page.mouse.up()
        self.assertEqual(await self.page.evaluate('window.getSelection().toString()'), text)
    ...
    async def test_trigger_hover(self):
        ...
        self.assertEqual(await self.page.evaluate('document.querySelector("button:hover").id'), 'button-6')
        await self.page.hover('#button-2')
        self.assertEqual(await self.page.evaluate('document.querySelector("button:hover").id'), 'button-2')
        await self.page.hover('#button-91')
        self.assertEqual(await self.page.evaluate('document.querySelector("button:hover").id'), 'button-91')

    @sync
    async def test_right_click(self):
        await self.page.goto(self.url + 'static/scrollable.html')
        await self.page.click('#button-8', button='right')
        self.assertEqual(await self.page.evaluate('document.querySelector("#button-8").textContent'), 'context menu')
    ...
    async def test_click_with_modifier_key(self):
        ...
        for key, value in modifiers.items():
            ...
            self.assertTrue(await self.page.evaluate('mod => window.lastEvent[mod]', value))
            await self.page.keyboard.up(key)
        await self.page.click('#button-3')
        for key, value in modifiers.items():
            self.assertFalse(await self.page.evaluate('mod => window.lastEvent[mod]', value))
    ...
    async def test_mouse_movement(self):
        ...
        self.assertEqual(
            await self.page.evaluate('window.result'),
            [
                [120, 140],
                [140, 180],
                [160, 220],
    ...
    async def test_touches_report(self):
        ...
        button = await self.page.J('button')
        await button.tap()
        self.assertEqual(await self.page.evaluate('getResult()'), ['Touchstart: 0', 'Touchend: 0'])
