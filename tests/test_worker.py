...
class TestWorker(BaseTestCase):
    ...
    async def test_create_destroy_events(self):
        ...
        workerObj = await self.page.evaluateHandle('() => new Worker("data:text/javascript,1")')
        worker = await workerCreatedPromise
        workerDestroyedPromise = asyncio.get_event_loop().create_future()
        self.page.once('workerdestroyed', lambda w: workerDestroyedPromise.set_result(w))
        await self.page.evaluate('workerObj => workerObj.terminate()', workerObj)
    ...
    async def test_jshandle_for_console_log(self):
        ...
        await self.page.evaluate('() => new Worker("data:text/javascript,console.log(1,2,3,this)")')
        log = await logPromise
        self.assertEqual(log.text, '1 2 3 JSHandle@object')
        self.assertEqual(len(log.args), 4)
        self.assertEqual(
            await (await log.args[3].getProperty('origin')).jsonValue(), 'null',
        )
